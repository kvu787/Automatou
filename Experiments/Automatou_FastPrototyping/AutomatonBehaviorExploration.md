...
The existing checkpoint/rewind system is valuable. Re-run from the same state with one altered parameter or relationship and inspect the difference. Preserve serializable physical state, beliefs, intentions, relationships, leases, and randomness. Test save/restore continuation for new state. Time-dependent updates such as cooling, forgetting, and field decay should run at declared simulation boundaries rather than depending on how often an inspector or brain calls a query.
