...
public readonly record struct Hex(int Q, int R) {
    ...
    public static IReadOnlyList<Hex> Directions { get; } = Array.AsReadOnly(new Hex[] { new(1, 0), new(0, 1), new(-1, 1), new(-1, 0), new(0, -1), new(1, -1) });
    public static IReadOnlyList<string> DirectionNames { get; } = Array.AsReadOnly(["East", "Northeast", "Northwest", "West", "Southwest", "Southeast"]);
