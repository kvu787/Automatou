...
public static class ScenarioCatalog
{
    ...
    {
        var entity = new Entity { Unit = unit, Faction = faction, Position = Hex.FromOffset(x, y), Facing = facing, Stationary = stationary };
        if (!world.Add(entity, out string reason)) throw new InvalidOperationException("Invalid experiment placement: " + reason);
        return entity;
    }

    private static World HeatAndReadiness()
    {
        var world = World.Create(false, 22, 20, false);
        var measured = Place(world, new SiegeWalker(), Faction.MechAndTank, 5, 14);
        measured.Unit.Brain.Settings = new() { Aggression = .55, Caution = .4, Commitment = .1, HeatReserve = 70 };
        Place(world, new TrainingTarget(), Faction.Prytu, 10, 14, 3, true);
        var aggressive = Place(world, new SiegeWalker(), Faction.MechAndTank, 5, 5);
        aggressive.Unit.Brain.Settings = new() { Aggression = .95, Caution = .4, Commitment = .1, HeatReserve = 100 };
        Place(world, new TrainingTarget(), Faction.Prytu, 10, 5, 3, true);
        for (int x = 1; x < 21; x++) world.Terrain[Hex.FromOffset(x, 10)] = Terrain.Paved;
    ...
    {
        var world = World.Create(false, 24, 16, false);
        var pursuer = Place(world, new CloneInfantry(), Faction.InfantryAndArtillery, 5, 7);
        var runner = Place(world, new TravelerOutrider(), Faction.Travelers, 9, 7);
        runner.Health = 24;
        runner.Unit.Brain.Settings = new() { Aggression = .1, Caution = 1, Commitment = .05, RememberContacts = false };
        for (int y = 2; y <= 13; y++)
            for (int x = 11; x <= 13; x++) world.Terrain[Hex.FromOffset(x, y)] = Terrain.Forest;
        ...
        if (world.CanObserve(pursuer, runner))
            pursuer.Unit.Brain.State.Contacts.Add(new ContactMemory { Id = runner.Id, Faction = runner.Faction, Position = runner.Position,
    ...
    {
        var world = World.Create(false, 25, 21, false);
        var guard = Place(world, new Bastion(), Faction.Bastions, 5, 10);
        var lower = Place(world, new TrainingTarget(), Faction.Bastions, 10, 6, 0, true);
        var upper = Place(world, new TrainingTarget(), Faction.Bastions, 10, 14, 0, true);
        ...
        foreach (int y in new[] { 6, 14 })
            for (int i = 0; i < 3; i++)
                Place(world, new CloneInfantry(), Faction.InfantryAndArtillery, 14 + i * 2, y, 3);
