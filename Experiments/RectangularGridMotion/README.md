...
3. **Corner pivot:** translate linearly, or rotate around the footprint's current world-space top-left corner. Integer corners ensure the resulting footprint stays aligned to the grid. The pivot is chosen anew on each command, so it is not a persistent physical corner; repeated turns can move the unit across the board.

The state is an axis-aligned occupied rectangle, not a vehicle with persistent heading. Accepted commands immediately update the unit. Rejected commands leave it in place and show a static red outline of the rejected destination.
