...
public static class HexGrid
{
    ...
    public static GridPoint Neighbor(GridPoint point, int direction)
    {
        ...
        var q = point.X - (point.Y - (point.Y & 1)) / 2;
    ...
    public static IEnumerable<GridPoint> Neighbors(GridPoint point, int width, int height)
    {
        ...
        {
            var neighbor = Neighbor(point, direction);
            if (Contains(neighbor, width, height)) yield return neighbor;
        }
    }
    ...
    {
        var leftQ = left.X - (left.Y - (left.Y & 1)) / 2;
        var rightQ = right.X - (right.Y - (right.Y & 1)) / 2;
        var dq = leftQ - rightQ;
